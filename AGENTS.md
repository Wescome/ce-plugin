...
**Runtime script invocations via the Bash tool — resolve against the project CWD, on every host:** When skill content tells the agent to *execute* a bundled script through the Bash tool, a bare relative path does **not** work — not on Claude Code, Codex, or Cursor. The Bash tool's working directory is the user's **project**, not the skill directory, on all three. So `bash scripts/my-script.sh` resolves to `<project>/scripts/…` and **fails loudly** (`exit 127`, no such file) — it does *not* silently skip; a capable model often recovers by re-issuing with the absolute skill-dir path, but that is a wasted round-trip and weaker models / mid-tier subagents botch it. This is a recurring bug class (#764 `ce-worktree`, #811 `ce-code-review`, #898 `ce-compound`). The agentskills.io standard's claim that "the agent runs commands from [the skill directory]" is **not** honored by these harnesses, so do not rely on it. The portable fix is to anchor to where the agent read the SKILL.md, so resolution is **deterministic** (the agent never has to derive a path or recover from an error):

```
SKILL_DIR="<absolute path of the directory containing the SKILL.md you just read>"
if [ -f "$SKILL_DIR/scripts/my-script.sh" ]; then
  bash "$SKILL_DIR/scripts/my-script.sh" ARG
else
  echo "ERROR: $SKILL_DIR/scripts/my-script.sh not found — re-check the SKILL.md path you read"
fi
```
