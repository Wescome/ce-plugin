...
## 2. Governance layer, in full (per `BRIEF-FF-CE-GOVERNANCE-FORK-001`)

The paragraph above is the shape; this section is the actual design brief for the governance layer (v1.0, "Verified on-device (macOS)"), which grounds the fork in a broader initiative (FunctionFactory) that the plugin-architecture analysis alone doesn't capture.
...
**Node types & identity model.** Nine node types are emitted — the SE-Onto continuant set the Factory ingests: `Specification`, `CandidateSet`, `ElucidationArtifact`, `Learning`, `Hypothesis`, `Amendment`, `Verdict`, `ExecutionTrace`, `Divergence`. Identity is `sha256(type ‖ canonical(content))`, first 32 hex — same content ⇒ same id ⇒ idempotent. Nodes are never overwritten; a re-write of the same source is a successor node carrying a `LINEAGE` edge to its predecessor. Store: `docs/.governance/<Type>-<hash>.json` plus a `.by-source.json` index resolving successor chains.
