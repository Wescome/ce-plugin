...
**AE1 and AE5 — confirmed live.** No `implementation-ready` plan in `docs/plans/` was actually unbuilt at run time (every existing candidate had already shipped), so a deliberately-flawed throwaway test-fixture plan (`docs/plans/2026-07-07-001-feat-fanout-smoke-grep-plan.md`, since deleted per its own Definition of Done) was authored and dispatched alongside `docs/plans/2026-07-02-001-feat-ce-sweep-skill-plan.md` (already-shipped, reused as the concurrent item rather than building a second throwaway feature). Result: the fixture's `ce-code-review` pass flagged a real P0 (unsanitized shell interpolation, `autofix_class: manual`), `emit-governance.mjs`'s `PostToolUse(Write)` hook created a genuine `refuted` Verdict node from that finding in the fixture's own worktree, and `hooks/strict-gate.mjs`'s `PreToolUse(Bash)` hook hard-blocked the subsequent `git commit` (`exit 2`, "1 unresolved refuted verdict(s)") — confirming R7's ship-gate mechanism fires for real, not just in `hooks/test-governance.mjs`'s mocked assertions. The dispatched `lfg` run correctly recognized this as a terminal `blocked` state and stopped cleanly — no hang, no retry loop, no PR opened (AE5). The Verdict node and the vulnerable script never left the fixture's own local worktree (confirmed: absent from `main`, absent from any pushed branch) — direct confirmation that R7's isolation is real at the physical-worktree level, not merely asserted. The concurrently-dispatched `ce-sweep` item independently reached its own terminal state (`blocked`, but for an unrelated reason — see below) without being affected by the fixture's block, in a separate worktree that never saw the fixture's Verdict file.

**Caveat — the "ships normally" half of AE1 was not demonstrated by the same paired item in this run.** The `ce-sweep` item stopped itself before ever attempting a `git commit` (it detected its own deliverable was already fully merged and declined to reimplement), so it never exercised the commit/push path at all — it did not "ship" in this run, and it did not test whether an unrelated item's commit *succeeds* while another is blocked. That positive half (a genuinely clean `shipped` terminal state, reached concurrently with a blocked item, in the same run) remains unobserved. Separately, this session's earlier real run (dispatching `ce-pov` + `ce-explain`, both already-shipped) did produce one genuine clean idempotent `shipped` terminal state — but not concurrently with a blocked item. **Net: the driver has never yet produced "one item blocked + one item shipped" as the outcome of the very same concurrent run** — accepted as a residual gap rather than re-run further, since manufacturing it requires either a second genuinely-unbuilt real plan or another throwaway fixture, and the core mechanism (per-item isolation via separate worktrees, hard block on refuted Verdict, clean terminal state under real contention) is now directly confirmed regardless.

**AE3 remains unobserved** — no pre-existing refuted Verdict was on `main` at any point this session (the one produced in this run stayed local to its own worktree, per KTD1/R7, and was deleted afterward), so the "inherits the block from a merged predecessor" path was never exercised. Deferred, per the Scope Boundaries note below.
...
- [x] U1-U3 land and `node hooks/test-governance.mjs` passes with the extended assertions (no regression on the existing 23) — 33 assertions, all pass.
- [ ] U5 is built dispatching `lfg` directly per item, exists at `.claude/workflows/concurrent-fanout.mjs`, and AE1, AE3, AE5 are each observed in a real run, not asserted from reading the code. **Partially done:** U5 is built and shipped (PR #1); AE1 and AE5 are confirmed live (see Verification Contract note above, including the "ships normally" caveat); AE3 remains unobserved — no pre-existing refuted Verdict has been on `main` to test the inherited-block path.
- [x] `.ce-fanout-plan` is added to `.gitignore`.
- [x] Dead-end code from whichever U4 branch was *not* chosen (the discarded spike script, or an abandoned hand-assembly attempt) is removed from the diff.
- [x] No progress/status field was added to this plan file — completion is derived from the checklist above and git, not stored in the doc.
