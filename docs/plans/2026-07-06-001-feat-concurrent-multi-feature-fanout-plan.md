...
| `lfg` ready-plan bypass validation | Run `skills/lfg/SKILL.md`'s new step-1 bypass through `/skill-creator`'s eval workflow (or re-test in a fresh session) with an already-`implementation-ready` plan fixture, confirming it skips `ce-plan` and proceeds straight to `ce-work` | This session's own edit to `skills/lfg/SKILL.md` cannot be exercised live from within the same session (plugin/skill content caches at session start, the same reason R8(b) needed a fresh session); U5 should not be built to depend on it live until this check passes |
...
- [ ] The ready-plan bypass is validated via `/skill-creator`'s eval workflow or a fresh session (Verification Contract) before U5 is built to depend on it live.
