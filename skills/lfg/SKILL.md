...
1. Determine whether `$ARGUMENTS` is already a ready plan before invoking `ce-plan`.

   Check whether `$ARGUMENTS` (trimmed) is a path to an existing file. If it is, read its frontmatter. When it already has `artifact_contract: ce-unified-plan/v1`, `artifact_readiness: implementation-ready`, and `execution: code`, treat `$ARGUMENTS` itself as the plan file path, skip invoking `ce-plan` entirely, and go straight to the GATE below. This bypass exists so a caller that already holds a ready plan (e.g. a batch/fan-out driver dispatching several plans at once) doesn't pay a redundant `ce-plan` resume/deepening pass on every item. Otherwise — the path doesn't exist, isn't a unified plan, isn't yet implementation-ready, or `$ARGUMENTS` is a feature description rather than a path — invoke the `ce-plan` skill with `$ARGUMENTS` as before.

   GATE: STOP. If ce-plan reported the task is non-software and cannot be processed in pipeline mode, stop the pipeline and inform the user that LFG requires software tasks. Otherwise, verify that a plan file in `docs/plans/` is available — either the one `ce-plan` just produced, or the pre-existing one the bypass above matched. If `ce-plan` ran and produced no plan file, invoke `ce-plan` again with `$ARGUMENTS`. Do NOT proceed to step 2 until a written plan exists. **Record the plan file path** — it will be passed to ce-work in step 2 and ce-code-review in step 4.

   Read the plan metadata before continuing (already read above when the bypass matched). If the plan has `artifact_contract: ce-unified-plan/v1`, proceed only when it has `artifact_readiness: implementation-ready` and `execution: code`. Stop the pipeline for `artifact_readiness: requirements-only`, any unrecognized readiness value, `execution: knowledge-work`, approach-plan outputs, answer-seeking/universal outputs, or invalid progress-like readiness values. LFG never launches `/goal` directly; when goal-mode or dynamic workflows are appropriate, `ce-work` owns that implementation engine choice and must return control to LFG afterward.
