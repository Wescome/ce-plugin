...
4. **Choose Execution Engine, then Strategy**
   ...
   For the inline/subagent engine, **prefer subagents for any structured multi-unit plan** — each worker gets a fresh context window for one unit, which is the main benefit. **Parallelize independent units whenever it is safe**, to go faster; fall back to serial only when parallel isn't safe or the harness can't isolate concurrent writes. Let the plan's `Dependencies` and `Files` drive batching: run an independent dependency layer together, then the next. The goal is to use workers aggressively where the plan allows — not to minimize them.
