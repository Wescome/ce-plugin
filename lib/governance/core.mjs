...
export function resolveSpecification(root, planRelPath) {
  if (planRelPath) {
    const idx = loadIndex(root)
...
export function emitExecutionTrace(root, cap, planRelPath) {
  const { governedBy, candidates } = resolveSpecification(root, planRelPath)
  const content = {
  ...
  return writeNode(root, "ExecutionTrace", content, `#worktree:${cap.branch}`)
}
