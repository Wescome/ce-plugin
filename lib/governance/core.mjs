...
// .claude/workflows/concurrent-fanout.mjs's dispatch prompt hardcodes this path
// ("docs/.governance/.by-source.json") as literal instruction text for a dispatched
// agent, since it can't import these constants — keep that string in sync if either
// changes.
 ...
 *  preloadedIdx lets a caller that already loaded .by-source.json (e.g. emitExecutionTrace,
 *  which resolves a Specification by path first) skip a redundant re-read. */
export function writeNode(root, type, content, sourceKey, preloadedIdx) {
  mkdirSync(GOV(root), { recursive: true })
  const id = governanceNodeId(type, content)
  const file = join(GOV(root), `${id.replace(":", "-")}.json`)
  const idx = preloadedIdx ?? loadIndex(root)
...
/** Resolve the governing Specification for a plan path via .by-source.json's existing
 *  path-keyed index, falling back to latestSpecification()'s repo-wide newest-inference
 *  when no path is given or it has no governed entry yet (legacy plans). Needed once
 *  concurrent worktrees can be mid-flight on different plans at once — "newest" stops
 *  identifying the right one. */
export function resolveSpecification(root, planRelPath, preloadedIdx) {
  if (planRelPath) {
    const idx = preloadedIdx ?? loadIndex(root)
    const id = idx[planRelPath]
    if (id && id.startsWith("Specification:")) return { governedBy: id, candidates: [id] }
  }
  return latestSpecification(root)
}

/** Emit an ExecutionTrace from a worktree capture. Idempotent on identical diff;
 *  a changed diff is a LINEAGE successor of the prior worktree trace. planRelPath,
 *  when given, resolves the governing Specification by path (resolveSpecification)
 *  instead of by repo-wide newest-inference. */
export function emitExecutionTrace(root, cap, planRelPath) {
  const idx = loadIndex(root) // loaded once, threaded through both the resolver and the write below
  const { governedBy, candidates } = resolveSpecification(root, planRelPath, idx)
  const content = {
    branch: cap.branch, head: cap.head, files: cap.files,
    diffSha: governanceNodeId("blob", cap.diff), stat: cap.stat,
    governedBy,                       // path-keyed when planRelPath resolves, else newest-inference
    governedByCandidates: candidates, // recoverable if the heuristic is wrong
  }
  // sourceKey is parameterized by branch so concurrent worktrees never collide on one
  // hardcoded key (each branch's ExecutionTrace stays independently addressable).
  return writeNode(root, "ExecutionTrace", content, `#worktree:${cap.branch}`, idx)
}
